...
class Tournament
{
    ...
    public function tally($scores){
        if(strlen($scores)>0){
            $this->table .= "\n";
             $this->splitGames(explode("\n",$scores));
        }
        return rtrim($this->table);
    }

    private function splitGames($games){
        for($i=0;$i<count($games);$i++){
            $this->makeObject(explode(";", $games[$i]));
        }
        $this->setPoints();
        $this->render();
    }

    private function makeObject($game){
        if(!array_key_exists($game[0],$this->map)){
            $this->map[$game[0]] = array("MP"=>0,"W"=>0,"D"=>0,"L"=>0,"P"=>0);
        }
        if(!array_key_exists($game[1],$this->map)){
            $this->map[$game[1]] = array("MP"=>0,"W"=>0,"D"=>0,"L"=>0,"P"=>0);
        }
        $this->setScore($game);
    }

    private function setScore($game){
        $this->map[$game[0]]['MP']++;
        $this->map[$game[1]]['MP']++;
        switch($game[2]){
            case "win":
    ...
    private function render(){
        ...
        $keys = array_keys($this->map);
        for($i=0;$i<count($keys);$i++){
            $search = array("*Team*","*MP*","*W*","*D*","*L*","*P*");
            $replace = array($this->formatName($keys[$i]),$this->map[$keys[$i]]["MP"],$this->map[$keys[$i]]["W"],$this->map[$keys[$i]]["D"],$this->map[$keys[$i]]["L"],$this->map[$keys[$i]]["P"] );
